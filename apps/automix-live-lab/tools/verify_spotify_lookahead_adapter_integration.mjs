...
// --- Wire an adapter through: seed -> observed -> runLookaheadCycle -> confirm -> advance -> second cycle ---
{
  ...
  // or SUCC2 could legitimately be chosen first -- the mock queue
  // reflects whichever the adapter actually enqueued, rather than
  // assuming one, so this test proves the real wiring, not a guess.
  let lastEnqueuedTrackId = null;
  adapter._api = async (url, opts = {}) => {
    ...
    if (url.startsWith("/me/player/queue?")) {
      ...
      if (uri) lastEnqueuedTrackId = uri.replace("spotify:track:", "");
      return null; // POST enqueue, 204-equivalent
    }
    if (url === "/me/player/queue") {
      return { currently_playing: { id: "SEED123" }, queue: lastEnqueuedTrackId ? [{ id: lastEnqueuedTrackId }] : [] };
    }
    return null;
  };
  ...
  feedState(adapter, fakeState({ trackId: "SEED123", artistId: "ART_SEED" }));
  ...
  const firstSuccessorId = lastEnqueuedTrackId; // "SUCC1" or "SUCC2" -- whichever the planner actually chose
  const firstSuccessorArtist = firstSuccessorId === "SUCC1" ? "ART_1" : "ART_2";
  const confirm1 = await adapter.confirmLookaheadSuccessor();
  check("confirmLookaheadSuccessor confirms via GET /me/player/queue", confirm1.confirmed === true);
  check("adapter status shows successorConfirmed true", adapter.getLookaheadStatus().successorConfirmed === true);
  ...
  const secondSuccessorId = lastEnqueuedTrackId;
  ...
  const confirm2 = await adapter.confirmLookaheadSuccessor();
  check("second successor confirms too", confirm2.confirmed === true);
  feedState(adapter, fakeState({ trackId: secondSuccessorId, artistId: secondSuccessorId === "SUCC1" ? "ART_1" : "ART_2" }));
