"""Run opaque owner-local tracks through canonical ``allin1.analyze``.

Private paths are read from an existing local mapping and are never emitted.
The caller must redirect the full upstream console stream to a gitignored log.
"""
...
def runtime_identity() -> dict:
    ...
    if observed["deterministic_control"]["sitecustomize_rng_seed"] != "0":
...
def checkpoint_identity(path_value: str) -> dict:
    ...
    return {
        ...
        "loader_consumed_cache_file_identity_recorded": True,
    }
...
def validate_result(result) -> tuple[dict, bool]:
    ...
    meter = max(beat_positions, default=None)
    return {
        "bpm": result.bpm,
        "beats_ms": beats_ms,
        ...
        "meter_evidence": {
            "maximum_beat_position": meter,
            "observed_positions": sorted(set(beat_positions)),
...
def analyze_one(opaque_id: str, source: Path, scratch: Path, order: int, run_index: int) -> dict:
    ...
    def captured_download(*args, **kwargs):
        path_value = original_download(*args, **kwargs)
    ...
    call_dir = scratch / f"{opaque_id}-run-{run_index}"
    started = time.perf_counter()
    try:
    ...
    except Exception as exc:  # private message remains only in redirected local log
        return {
            "execution_order": order,
            "opaque_id": opaque_id,
            "run_index": run_index,
...
def main() -> int:
    ...
    parser.add_argument("--track-ids", nargs="+", required=True)
    parser.add_argument("--repeats", type=int, default=1)
    parser.add_argument("--start-order", type=int, required=True)
    parser.add_argument("--prior-smoke")
    parser.add_argument("--scratch", required=True)
    parser.add_argument("--out", required=True)
    args = parser.parse_args()

    frontier = json.loads(Path(args.frontier).read_text(encoding="utf-8"))
    allowed = set(frontier["derived_track_ids"])
    if len(set(args.track_ids)) != len(args.track_ids):
        raise ValueError("DUPLICATE_TRACK_ARGUMENT")
    if not set(args.track_ids).issubset(allowed):
        raise ValueError("TRACK_OUTSIDE_DERIVED_FRONTIER")
    if not all(OPAQUE_ID.fullmatch(value) for value in args.track_ids):
        raise ValueError("INVALID_OPAQUE_ID")

    if args.prior_smoke:
        smoke = json.loads(Path(args.prior_smoke).read_text(encoding="utf-8"))
        smoke_runs = smoke["runs"]
        if [row["opaque_id"] for row in smoke_runs] != ["RM014", "RM014"]:
            raise ValueError("RM014_NOT_FIRST_TWICE")
        if not all(row["execution_result"] == "PASS" and row["output_schema_valid"] for row in smoke_runs):
            raise ValueError("RM014_SMOKE_NOT_ACCEPTED")
    ...
    for opaque_id in args.track_ids:
        if opaque_id not in mapping:
            raise ValueError("OPAQUE_ID_MISSING_FROM_APPROVED_MAPPING")
        source = windows_to_wsl(str(mapping[opaque_id]))
        if not source.is_file():
            raise ValueError("APPROVED_MAPPED_AUDIO_MISSING")
        for run_index in range(1, args.repeats + 1):
            row = analyze_one(opaque_id, source, scratch, order, run_index)
            runs.append(row)
            order += 1
            if row["execution_result"] != "PASS":
                break
        if runs[-1]["execution_result"] != "PASS":
            break
    ...
    return 0 if all(row["execution_result"] == "PASS" for row in runs) else 2
